...
void QtGui::paintEvent(QPaintEvent * ev)
{
	...
	{
		auto & item = saved_data[ui.parts_list->item(i)];
...
void QtGui::on_parent_combo_currentIndexChanged(int row)
{
	...
	saved_data[this_item].parent = parent_item;
}
...
void QtGui::on_parts_list_currentRowChanged(int row)
{
	...
	else
	{
		...
		{ // Parent Combo
			...
			for (int i = 0; i < ui.parts_list->count(); ++i)
			{
				...
				if (this_item != target_item)
				{
					...
					if (target_item == saved_data[this_item].parent)
					{
						parent_finded = true;
				...
				else
				{
					...
					if (saved_data[this_item].parent == nullptr)
					{
						parent_finded = true;
						ui.parent_combo->setCurrentIndex(i);
					}
			...
			if (!parent_finded)
			{
				for (int i = 0; i < ui.parts_list->count(); ++i)
				{
					...
					if (this_item == target_item)
					{
						saved_data[this_item].parent = nullptr;
						ui.parent_combo->setCurrentIndex(i);
					}
				}
			}
		...
		{ // Transform Box
			ui.position_x->setValue(static_cast<double>(saved_data[this_item].position.x));
			ui.position_y->setValue(static_cast<double>(saved_data[this_item].position.y));
			ui.position_z->setValue(static_cast<double>(saved_data[this_item].position.z));

			ui.rotation_x->setValue(static_cast<double>(saved_data[this_item].rotation.x));
			ui.rotation_y->setValue(static_cast<double>(saved_data[this_item].rotation.y));
			ui.rotation_z->setValue(static_cast<double>(saved_data[this_item].rotation.z));

			ui.scale_x->setValue(static_cast<double>(saved_data[this_item].scale.x));
			ui.scale_y->setValue(static_cast<double>(saved_data[this_item].scale.y));
			ui.scale_z->setValue(static_cast<double>(saved_data[this_item].scale.z));
		}

		{ // Primitive Type
			ui.primitive_type->setCurrentIndex(saved_data[this_item].primitive_type);
			
			ui.plane_div_x->setValue(saved_data[this_item].plane_div_x);
			ui.plane_div_y->setValue(saved_data[this_item].plane_div_y);
			ui.plane_size_x->setValue(saved_data[this_item].plane_size.x);
			ui.plane_size_y->setValue(saved_data[this_item].plane_size.y);

			ui.box_size_x->setValue(saved_data[this_item].box_size.x);
			ui.box_size_y->setValue(saved_data[this_item].box_size.y);
			ui.box_size_z->setValue(saved_data[this_item].box_size.z);

			ui.sphere_diameter->setValue(saved_data[this_item].sphere_diameter);
			ui.sphere_tesselation->setValue(saved_data[this_item].sphere_tesselation);
		}
...
{
	saved_data[ui.parts_list->currentItem()].position.x = static_cast<float>(value);
}

void QtGui::on_position_y_valueChanged(double value)
{
	saved_data[ui.parts_list->currentItem()].position.y = static_cast<float>(value);
}

void QtGui::on_position_z_valueChanged(double value)
{
	saved_data[ui.parts_list->currentItem()].position.z = static_cast<float>(value);
}

void QtGui::on_rotation_x_valueChanged(double value)
{
	saved_data[ui.parts_list->currentItem()].rotation.x = static_cast<float>(value);
}

void QtGui::on_rotation_y_valueChanged(double value)
{
	saved_data[ui.parts_list->currentItem()].rotation.y = static_cast<float>(value);
}

void QtGui::on_rotation_z_valueChanged(double value)
{
	saved_data[ui.parts_list->currentItem()].rotation.z = static_cast<float>(value);
}

void QtGui::on_scale_x_valueChanged(double value)
{
	saved_data[ui.parts_list->currentItem()].scale.x = static_cast<float>(value);
}

void QtGui::on_scale_y_valueChanged(double value)
{
	saved_data[ui.parts_list->currentItem()].scale.y = static_cast<float>(value);
}

void QtGui::on_scale_z_valueChanged(double value)
{
	saved_data[ui.parts_list->currentItem()].scale.z = static_cast<float>(value);
}

void QtGui::on_primitive_type_currentChanged(int value)
{
	saved_data[ui.parts_list->currentItem()].primitive_type = value;
...
{
	saved_data[ui.parts_list->currentItem()].plane_div_x = value;
...
{
	saved_data[ui.parts_list->currentItem()].plane_div_y = value;
...
{
	saved_data[ui.parts_list->currentItem()].plane_size.x = value;
...
{
	saved_data[ui.parts_list->currentItem()].plane_size.y = value;
...
{
	saved_data[ui.parts_list->currentItem()].box_size.x = value;
...
{
	saved_data[ui.parts_list->currentItem()].box_size.y = value;
...
{
	saved_data[ui.parts_list->currentItem()].box_size.z = value;
...
{
	saved_data[ui.parts_list->currentItem()].sphere_diameter = value;
...
{
	saved_data[ui.parts_list->currentItem()].sphere_tesselation = value;
...
{
	auto & item = saved_data[ui.parts_list->currentItem()];
...
void QtGui::on_add_button_pressed(void)
{
	char str[80];
	static int n = 0;
	sprintf(str, "%3d: %s", n++, "Empty Object");
