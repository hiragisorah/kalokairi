...
void ResultRenderingSystem::Initialize(void)
{
	...
	this->title_tex_ = graphics->CreateTexture("data/texture/howto.png");
...
void ResultRenderingSystem::Always(void)
{

}

void ResultRenderingSystem::Begin(Seed::Graphics & graphics)
{
	if (start_cnt > 0)
	{
...
void ResultRenderingSystem::Render(Seed::Graphics & graphics)
{
	if (Seed::Input::Trigger(VK_SPACE))
	{
		end_cnt = 90;
		this->owner()->engine()->ChangeScene<MainScene>();
	}
...
void ResultRenderingSystem::End(Seed::Graphics & graphics)
{
	...
	mtx *= DirectX::Matrix::CreateTranslation(DirectX::Vector3(0.7f, -0.7f, 0));
...
void ResultRenderingSystem::AddModel(HierarchyModel * const model)
{
	if (model->primitive_id() != -1)
		this->model_list_.emplace_back(model);
}

void ResultRenderingSystem::Rendering(Seed::Graphics & graphics, HierarchyModel * const model)
{
