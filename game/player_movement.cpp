...
void PlayerMovement::Update(void)
{
	...
	float move_forward = static_cast<float>(Input::Press('W')) - static_cast<float>(Input::Press('S')) * 0.5f;
	
	float move_side = static_cast<float>(Input::Press('D')) - static_cast<float>(Input::Press('A')) * 0.5f;

	float rot_side = static_cast<float>(Input::Press('E')) - static_cast<float>(Input::Press('Q'));
	...
	if (Input::Repeat('W') || Input::Repeat('A') || Input::Repeat('S') || Input::Repeat('D'))
		run = true;
